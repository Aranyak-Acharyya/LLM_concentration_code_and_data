####CODE FOR OBTAINING THE BOXPLOTS IN SIMULATION SECTION
...
print(c(mean(L),upbd,mean(ifelse(L<upbd,1,0))))
...
#1st iteration
fin_mat<-matrix(nrow=2+length(L),)

fin_mat<-cbind(fin_mat,dec)

df<-data.frame(fin_mat)

print(df)

save(df,file="total_improved_K5.RData")
...
load("total_improved_K5.RData")

print(df)

df<-cbind(store_as_matrix(df),dec)
...
dff<-data.frame(tr_vec,ddf[2:51,1],ddf[2:51,2],ddf[2:51,3],
               ddf[2:51,4])
print(dff)
dffm<-melt(dff, id.var='tr_vec')
print(dffm)

main_data<-data.frame(
  Group=rep(c("A","B","C","D"),each=50),
  Value=c(ddf[2:51,1],ddf[2:51,2],ddf[2:51,3],ddf[2:51,4])
)
...
target_points <- data.frame(
  Group = c("A","B","C","D"),
  TargetValue = c(ddf[52,1], ddf[52,2], ddf[52,3],ddf[52,4]) # The specific points you want to add
)
...
g<-ggplot(main_data, aes(x = Group, y = Value, colour = Group)) +
  geom_boxplot() +
  geom_point(data = target_points, aes(x = Group, y = TargetValue), 
             color = "black", size = 4, shape = 18) + 
  scale_colour_manual(values = c("red", "brown", "blue","darkgreen"),
                    labels=unname(TeX(c("$n=5$",
                                        "$n=10$",
                                        "$n=15$",
                                        "$n=20$"
                    )))) +
  labs(x="number of models (n)",
       y="estimation errors")

g


ggsave(g,file="LLM_conc_plot_simK3.pdf",
       width = 8, height = 4,
       units = "in", dpi = 600)
...
dff2<- data.frame()
g<-ggplot(data = dffm, aes(x=variable, y=value, fill = variable)) +
  geom_boxplot() +
  theme(legend.title = element_blank())  +
  scale_fill_manual(values = c("yellow", "orange", "green","skyblue"),
                    labels=unname(TeX(c("$n=5$",
                                        "$n=10$",
                                        "$n=15$",
                                        "$n=20$"
